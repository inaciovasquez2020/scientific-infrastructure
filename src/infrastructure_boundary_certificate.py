...
class InfrastructureBoundaryCertificate:
    theorem_id: str
    status: str
    required_count: int
    present_count: int
...
def infrastructure_boundary_certificate(root: Path | str, required: Sequence[str], boundary_text: str) -> InfrastructureBoundaryCertificate:
    normalized = _normalize_required(required)
    ...
    return InfrastructureBoundaryCertificate(
        theorem_id="SI-IBC-1",
